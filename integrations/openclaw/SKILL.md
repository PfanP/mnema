          ...
          [
            {
              "name": "MNEMA_TOKEN",
              "required": false,
              "description": "Bearer token printed by Mnema on first start. Auto-populated during install.",
            },
          ],
        "install":
          [
...
# Mnema — Memory That Actually Sticks

Every session starts from zero. You re-explain your stack, your preferences,
the decisions you made last week. Mnema fixes that.

Mnema sits between OpenClaw and your LLM provider and learns from your
...
If you maintain USER.md or MEMORY.md by hand, Mnema replaces both.
...
If the port is in use, tell the user to free it or set MNEMA_PORT to
...
mkdir -p "$HOME/.mnema"
docker run --rm \
  -v "$HOME/.mnema:/app/.mnema" \
  mnemaai/mnema:latest init
```

On Windows:

```powershell
New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.mnema" | Out-Null
docker run --rm `
  -v "$env:USERPROFILE\.mnema:/app/.mnema" `
  mnemaai/mnema:latest init
```
...
```bash
docker compose -f "$HOME/.mnema/docker-compose.yml" \
  --env-file "$HOME/.mnema/.env" up -d
```

On Windows:

```powershell
docker compose -f "$env:USERPROFILE\.mnema\docker-compose.yml" `
  --env-file "$env:USERPROFILE\.mnema\.env" up -d
```

### Step 5 — Wait for Mnema to be ready
...
```bash
MNEMA_PORT="${MNEMA_PORT:-5757}"
ATTEMPTS=0
until curl -sf "http://localhost:${MNEMA_PORT}/healthz" >/dev/null 2>&1 \
  || [ $ATTEMPTS -ge 30 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 2
done
...
```powershell
$port = if ($env:MNEMA_PORT) { $env:MNEMA_PORT } else { "5757" }
$attempts = 0
$ready = $false
...
If the result is "timeout", tell the user Mnema may still be
initializing and suggest checking logs:

```bash
docker compose -f "$HOME/.mnema/docker-compose.yml" logs mnema
```
...
```bash
docker compose -f "$HOME/.mnema/docker-compose.yml" \
  exec mnema node dist/index.js token
```
...
```bash
docker compose -f "$HOME/.mnema/docker-compose.yml" \
  run --rm mnema node dist/index.js token
```

On Windows replace $HOME/.mnema with $env:USERPROFILE\.mnema in
both commands above.
...
and discovers whatever the user has configured in Mnema.
...
- Mnema is running at http://localhost:5757
- Their existing provider is still configured and will be used as
  fallback. The setup is fully reversible by removing the mnema
...
## The !mnema slash command

When the user runs `!mnema` with no arguments, check status and
report back.
...
If the response contains `"ok":true`, report Mnema is running.
If the request fails, report Mnema is not reachable and suggest:
...
- Current provider config: `openclaw config get models.providers.mnema`
- Link to http://localhost:5757/beliefs to view the world model

When the user runs `!mnema onboarding`, tell them to open
...
When the user runs `!mnema update`:

```bash
docker compose -f "$HOME/.mnema/docker-compose.yml" pull
docker compose -f "$HOME/.mnema/docker-compose.yml" up -d
```

## Session commands

Mnema intercepts these commands directly in chat. No slash command setup
...
still injected into every session — Mnema just stops recording new ones.
...
- Mnema runs entirely locally. Nothing leaves the user's machine.
