# Mnema: Local Long-Term Memory for Open WebUI, LM Studio, and Any OpenAI-Compatible Client
...
Mnema is a local, privacy-first proxy that builds a structured world model of your preferences, decisions, and expertise, injecting the right context into every session
automatically. Point any OpenAI-compatible client at `localhost:5757` and every response is already contextualized. Your client doesn't know Mnema exists.

- **Zero Configuration**: Drop-in OpenAI API compatibility.
- **Local-First**: Your memory stays on your machine, no cloud, no tracking.
- **Beyond RAG**: Similarity search returns everything in the neighborhood because your beliefs genuinely are semantically related. Mnema uses alias-weighted term matching that returns exactly what you named, not everything nearby.

## Features

- **Zero re-briefing**: Your stack, decisions, and preferences carry forward automatically. Mnema ensures every new session already knows your work, eliminating the "Monday morning" context reset.
- **Instant import**: Drop in an existing skills file, bio, or notes doc and Mnema seeds your world model immediately, no cold start, no manual entry.
- **Transparent to your tools**: Point any OpenAI-compatible client at `localhost:5757/v1` and it works; no plugins, no custom integrations. The client doesn't know Mnema exists.
...
With Mnema, the same question in a new session, cold start, produced this:
...
Mnema sits between any OpenAI-compatible client and upstream LLM providers. On every request:

1. Assembles a **belief context**: a curated slice of your world model, budgeted within a token ceiling
2. Injects both into the system prompt alongside a sidecar instruction, a structured metadata block the model writes back, which Mnema strips before returning the response to your client
...
hand. Mnema extracts conclusions at write time, when the model already knows
...
Mnema sets your scope automatically from your first message. You can also set it explicitly:
...
Without scope, Mnema still works — it just retrieves across all your beliefs, which can introduce noise when your domains are very different from each other.
...
Mnema is conservative by design. It would rather surface nothing than surface the wrong thing. Retrieval quality improves over time because every session teaches the system
