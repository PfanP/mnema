...
Mnema encrypts the `content`, `why_it_matters`, and `source_exchanges` fields
...
Encryption is backed by two keys in `~/.mnema/`:
...
Mnema holds the keys; MongoDB never sees them.

## Backup and portability

Because Mnema is fully local, there is no cloud backup to fall back on. If you
...
same loss model as losing your `~/.ssh/` directory without a backup. Mnema
