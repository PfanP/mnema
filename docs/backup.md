...
Your world model lives entirely on your machine. Mnema never phones home, which
...
On the new machine, start Mnema and open `http://localhost:5757/admin/backup`.
...
```bash
ARCHIVE=$(base64 < ~/mnema-backup.enc)
...
```powershell
$archive = [Convert]::ToBase64String([IO.File]::ReadAllBytes("$env:USERPROFILE\mnema-backup.enc"))
...
If your disk fails without a backup, your world model is unrecoverable. Mnema's
