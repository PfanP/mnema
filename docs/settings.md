...
Manage Mnema at `http://localhost:5757/admin`. Changes take effect immediately unless noted.

## Providers

Connect the upstream LLM providers Mnema routes requests to. Mnema supports two direct providers and any OpenAI-compatible endpoint.
...
Mnema learns about you by extracting beliefs from your conversations. This
...
The toggle in Settings pauses extraction everywhere. When off, Mnema still
...
When you send `!extract off`, Mnema responds with a confirmation. Your
...
Pausing extraction does not make Mnema forget anything. It does not clear
...
Mnema injects relevant beliefs from your world model into each session as context. Injection can be controlled at the session or global level without touching Settings.
...
**Automatic scope detection**: when enabled, Mnema infers the domain from your first message each session and sets scope automatically. When disabled, scope is only set via explicit `!scope` commands or client metadata. In explicit-only mode, sessions without a declared scope surface only `user:universal` beliefs.
...
When you set a sub-domain scope such as `domain:code/typescript`, Mnema automatically expands the hierarchy to include parent scopes, so you do not need to list them separately.
...
Your bearer token for authenticating requests to Mnema. This is the token you paste into your client's API settings.

**Rotate token**: generates a new token immediately and invalidates the current one. Your active session will end. Copy the new token before dismissing the confirmation dialog: it is also saved to `~/.mnema/token` (Linux/macOS) or `%USERPROFILE%\.mnema\token` (Windows).
