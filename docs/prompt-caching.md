...
Mnema structures its system prompt in tiers specifically to maximize prompt cache hits across all supported models. This document explains how the tiers work, what the minimum token thresholds are, and why the prompts are written the way they are.
...
Mnema uses two cached blocks and one uncached section per request:
...
For LiteLLM and Bedrock Access Gateway setups, configure the endpoint flavor in the admin UI so Mnema sends the correct caching hints for your gateway. See [docs/clients.md](clients.md) for setup instructions by client type.
