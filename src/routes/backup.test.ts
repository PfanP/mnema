...
import type { MnemaExport } from "../backup/types.js";
...
test("POST /v1/backup/export returns encrypted binary archive", async (t) => {
  ...
  t.regex(
    ...
    /attachment.*mnema-backup.*\.enc/,
  );
  t.truthy(res.headers["x-mnema-export-version"]);
...
test("POST /v1/backup/export produces archive that can be decrypted", async (t) => {
  ...
  const payload = JSON.parse(decrypted.toString("utf-8")) as MnemaExport;
...
function makeExportPayload(): MnemaExport {
  return {
    version: 1,
