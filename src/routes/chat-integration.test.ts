...
test("response includes mnema metadata", async (t) => {
  ...
  const body = JSON.parse(res.body);
  t.truthy(body.mnema);
  t.is(body.mnema.session_id, SESSION_ID);
  t.truthy(body.mnema.turn_id);
  t.deepEqual(body.mnema.scope, ["work"]);
  t.is(body.mnema.parse_status, "missing");
});
...
test("parse_status is 'parsed' when sidecar is well-formed", async (t) => {
  ...
  const body = JSON.parse(res.body);
  t.is(body.mnema.parse_status, "parsed");
});
...
test("turn and job share the same turnId", async (t) => {
  ...
  const { mnema } = JSON.parse(res.body);

  const turn = await waitForDoc(db.collection("turns"), {
    _id: mnema.turn_id,
  });
  const job = await waitForDoc(db.collection("jobs"), {
    turn_id: mnema.turn_id,
  });
  t.truthy(turn);
  t.truthy(job);
...
test("scope from metadata takes precedence over session activeScope", async (t) => {
  ...
  const body = JSON.parse(res.body);
  t.deepEqual(body.mnema.scope, ["override-scope"]);
});
...
test("falls back to session activeScope when metadata scope is absent", async (t) => {
  ...
  const body = JSON.parse(res.body);
  t.deepEqual(body.mnema.scope, ["session-scope"]);
});
...
test("!scope command mnema reflects new scope immediately", async (t) => {
  ...
  const body = JSON.parse(res.body);
  t.deepEqual(body.mnema.scope, ["domain:code", "domain:writing"]);
});

test("!scope command mnema parse_status is missing since no LLM fired", async (t) => {
  const { app } = await buildApp(makeProviderResponse("Unused."));
...
test("!scope command mnema degraded is false", async (t) => {
  const { app } = await buildApp(makeProviderResponse("Unused."));
...
test("scope command mid-session changes scope for subsequent turn", async (t) => {
  ...
  const body = JSON.parse(res.body);
  t.deepEqual(body.mnema.scope, ["domain:writing"]);
});
...
test("first-turn scope detection runs when scopeDetector is configured and activeScope is empty", async (t) => {
  ...
  t.deepEqual(body.mnema.scope, ["domain:code"]);
...
test("first-turn scope detection failure degrades gracefully and still returns response", async (t) => {
  ...
  t.deepEqual(JSON.parse(res.body).mnema.scope, []);
});
...
test("!extract command mnema parse_status is missing since no LLM fired", async (t) => {
  const { app } = await buildApp(makeProviderResponse("Unused."));
...
test("!extract command mnema degraded is false", async (t) => {
  const { app } = await buildApp(makeProviderResponse("Unused."));
...
test("!inject command mnema parse_status is missing since no LLM fired", async (t) => {
  const { app } = await buildApp(makeProviderResponse("Unused."));
