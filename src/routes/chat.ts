...
export function registerChatRoute(app: FastifyInstance, deps: ChatDeps): void {
  app.post<{ Body: ChatBody }>("/v1/chat/completions", async (req, reply) => {
    ...
    if (intercepted) {
      if (tierWarning) reply.header("x-mnema-warning", tierWarning);
      return reply.send({
        id: `chatcmpl-${turnId}`,
    ...
    //
    // The X-Mnema-No-Extract header [17] provides per-request extraction
    // suppression without touching session or global state.
    const noExtractHeader =
      req.headers["x-mnema-no-extract"] === "true" ||
      req.headers["x-mnema-no-extract"] === "1";
...
async function handleStreamingResponse(
  ...
  const responseHeaders: Record<string, string> = {
    ...
    ...(ctx.tierWarning ? { "x-mnema-warning": ctx.tierWarning } : {}),
  };
